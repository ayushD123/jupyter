...
# Create a DataFrame 'X' with iris features (sepal length, sepal width, petal length, petal width)
X = pd.DataFrame(iris.data)
X.columns = ['Sepal_Length','Sepal_Width','Petal_Length','Petal_Width']

# Create a DataFrame 'Y' with iris target labels (species)
...
# Real Clustering Visualization
plt.subplot(1,2,1)
# Scatter plot for real clusters (species) based on petal length and width
plt.scatter(X.Petal_Length, X.Petal_Width, c=colormap[Y.Targets])
plt.title('Real Clustering')

# K-Means Clustering
model1 = KMeans(n_clusters=3)  # Create KMeans model with 3 clusters
model1.fit(X)  # Fit KMeans model using the features

plt.subplot(1,2,2)
# Scatter plot for KMeans clusters based on petal length and width
plt.scatter(X.Petal_Length, X.Petal_Width, c=colormap[model1.labels_], s=40)
plt.title('K Mean Clustering')
plt.show()

# EM (Expectation-Maximization) Clustering
model2 = GaussianMixture(n_components=3)  # Create Gaussian Mixture model with 3 components
model2.fit(X)  # Fit EM model using the features

plt.subplot(1,2,1)
# Scatter plot for EM clusters based on petal length and width
plt.scatter(X.Petal_Length, X.Petal_Width, c=colormap[model2.predict(X)], s=40)
plt.title('EM Clustering')
plt.show()

# Print actual target labels, K-Means labels, EM predicted labels, and accuracy scores
